...
const InicioSesion = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(!expresionRegular.test(correo)){
            ...
            cambiarAlerta({tipo:'error', mensaje:'Por favor, introduce un correo válido'})
            return;
        }

        if(correo === '' || password === ''){
            cambiarEstadoAlerta(true)
            cambiarAlerta({tipo:'error', mensaje:'Por favor, rellene todos los datos'})
            return;
        }
    ...
    return ( 
        <>
            ...
            <Alerta tipo={alerta.tipo} mensaje={alerta.mensaje} estadoAlerta={estadoAlerta} cambiarEstadoAlerta={cambiarAlerta} />  
        </>
     );
}
